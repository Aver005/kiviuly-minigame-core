...
- **Платформа** `mg-api` + `mg-core`: движок матча, реестр арен (по папкам игр),
  тулкит, меню, статистика (generic-счётчики), реестр игр, команды с привязкой к игре, права,
  оффлайн-возврат (встроенный `ExitGuard` по конфигу `Minigame.exitGuard()` + ручные хуки). Собирается.
- **SkyWars** — тонкий плагин, свой `/sw`, право `sw.admin`; **включён встроенный ExitGuard**
  (`config.yml` → `exit-guard.enabled/grace-seconds`, дефолт 60с). Собирается.
- **SkyBlockWars** — то же, `/sbw`, `sbw.admin`; **включён встроенный ExitGuard** (config, 60с).
  Нюанс: у SBW свои эпоха-респ-блоки для ОНЛАЙН-смертей; оффлайн-игрок при гибели болванчика выбывает
  сразу (без эпоха-респауна) — это осознанно (оффлайн играть не может; грейс даёт вернуться). Собирается.
