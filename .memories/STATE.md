...
2. **Punch-list мелких регрессий** (ядро перехватывает часть подкоманд ДО игрового хука):
   `/escape` без арг → ядровое меню; `/escape stats` бедное (4 строки vs 12); `/escape debuglog` →
   ядровый лог, escape-лог без переключателя; `/escape join` минует escape busy-проверку. **Решение владельца.**
3. **Оффлайн-страж rejoin** — фичи «вернуться в идущий матч» у ядра нет; сейчас только снапшот-восстановление.
4. **Статистика** escape всё ещё своя (`stats.db`, колонки games_played/mvp_games/quests/…) — не сквозная.
   Перевод в общий `StatsService` — отдельная задача (с ней же чинится бедный `/escape stats`).
