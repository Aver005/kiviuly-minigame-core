...
- **Оффлайн-возврат:** живого при дисконнекте игра может оставить в матче (`keepOnDisconnect`→true) —
  ядро не снимает снапшот и держит его в ростере; `onPlayerDisconnect`/`onPlayerReconnect` — точки для
  стража/возврата. Выбывший оффлайн остаётся спектатором в ростере, `cleanup` вернёт снапшот при след. входе.
