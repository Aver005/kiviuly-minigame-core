...
- **Выбывание ОБЯЗАНО идти через `match.eliminate` — иначе матч не кончится.** `isPlaying`/`aliveCount`/
  `defaultResult` читают ядровый `MatchPlayer.isAlive()`; сбросить его может только `eliminate`. Игра,
  что сама объявляет смерть, зовёт ТИХИЙ `match.eliminate(UUID)` (без broadcast движка, работает и для
  оффлайн-игрока), а чат-переход/спектейт получает через хук `onPlayerEliminated`. НЕ дублируй чат вручную.
- **Оффлайн-возврат — ДВА пути.** (1) **Встроенный страж выхода** (easy, opt-in): игра возвращает
  `ExitGuardConfig` из `Minigame.exitGuard()` (напр. `ExitGuardConfig.standard(90)`), и движок сам ведёт
  всё — держит игрока оффлайн-участником, ставит болванчика в его экипировке, грейс/возврат/заочную
  гибель (`GameSession.ExitGuard`, `GameListener.onEntityDeath` ловит убийство болванчика). Коду игры —
  одна строка. (2) **Ручные хуки** (для богатой game-специфики, как escape с respawn-блоками):
  `keepOnDisconnect`→true + `onPlayerDisconnect`/`onPlayerReconnect`. Движок сперва пробует конфиг-страж,
  потом ручной путь — они сосуществуют. Выбывший оффлайн остаётся спектатором; `onReconnect` чистит его
  дублирующий инвентарь, `cleanup` вернёт снапшот при след. входе.
